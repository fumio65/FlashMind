...
// ── Edit Class Dialog (separate component for clean keyboard handling) ──
...
export default function ClassDetailPage() {
  ...
  // Ctrl+E → open edit class dialog
  useEffect(() => {
    const handler = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'e') {
        e.preventDefault()
        if (!showEditClass && !showEditDeck && !toDeleteDeck && cls) {
          handleOpenEditClass()
        }
      }
    }
    window.addEventListener('keydown', handler, true)
    return () => window.removeEventListener('keydown', handler, true)
  }, [showEditClass, showEditDeck, toDeleteDeck, cls])
  ...
  // Delete Deck dialog: Esc = keep, Q = confirm delete
  useEffect(() => {
            ...
            <Card key={deck._id} className="border-border hover:border-primary/50 hover:shadow-md transition-all group">
              <CardContent className="p-5 flex flex-col gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm" variant="outline" className="px-3"
                  ...
                  {editingDeck.cards.slice(0, 4).map((card, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        i < Math.min(editingDeck.cards.length, 4) - 1 && 'border-b border-border'
                      )}
                    >
