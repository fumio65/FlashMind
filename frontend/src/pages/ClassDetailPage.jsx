...
import {
  updateClass, updateDeck,
  deleteDeck, deleteClass,
...
function EditClassDialog({
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "s") { e.preventDefault(); e.stopPropagation(); onSaveRef.current() }
    }
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="rounded-xl overflow-hidden border border-border shadow-sm">
              ...
              <div className="bg-card px-4 py-3">
                ...
                <div className="flex items-center gap-1 mt-2 text-[10px] text-muted-foreground">
                  {editPublic ? <><Globe className="h-3 w-3" />Public</> : <><Lock className="h-3 w-3" />Private</>}
                </div>
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {CLASS_COLORS.map((c) => (
                  <button
                    ...
                    onClick={() => setEditColor(c.value)} title={c.label}
                    className={cn(
                      "h-8 w-full rounded-lg bg-gradient-to-br transition-all relative", c.value,
                      editColor === c.value ? "ring-2 ring-offset-1 ring-primary scale-105" : "hover:scale-105",
                    )}
                  >
                    {editColor === c.value && <Check className="h-3.5 w-3.5 text-white absolute inset-0 m-auto" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              ...
              <Input value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="e.g. Data Structures, Calculus" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-foreground">
                Description <span className="text-muted-foreground font-normal ml-1 text-xs">(optional)</span>
              </label>
              <textarea
                value={editDesc} onChange={(e) => setEditDesc(e.target.value)}
                placeholder="Topics and lessons covered in this subject" rows={3}
            ...
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border">
              ...
              <button
                type="button" onClick={() => setEditPublic((v) => !v)}
                className={cn("relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none shrink-0", editPublic ? "bg-primary" : "bg-muted-foreground/30")}
              >
                <span className={cn("inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-transform", editPublic ? "translate-x-6" : "translate-x-1")} />
              </button>
            </div>
...
export default function ClassDetailPage() {
  ...
  const { cls, setCls, decks, setDecks, isLoading, error, refetch } = useClass(id)
  ...
  const isOwner = cls
  ? (
      cls.owner?._id?.toString() === currentUser?._id?.toString() ||
      cls.owner?.toString()      === currentUser?._id?.toString()
    )
  : false
  ...
  const handleSaveClass = async () => {
    setCls((prev) => ({ ...prev, name: editName, description: editDesc, icon: editIcon, color: editColor, isPublic: editPublic }))
    setShowEditClass(false)
    try {
      await updateClass(id, { name: editName, description: editDesc, icon: editIcon, color: editColor, isPublic: editPublic })
      refetch()
    } catch (err) {
  ...
  const handleSaveDeck = async () => {
    ...
    setDecks((prev) => prev.map((d) =>
      d._id === editingDeck._id
        ? { ...d, title: deckTitle, description: deckDesc, isPublic: deckPublic }
        : d
    ))
    setShowEditDeck(false)
    setEditingDeck(null)
    try {
      await updateDeck(editingDeck._id, { title: deckTitle, description: deckDesc, isPublic: deckPublic })
      refetch()
    } catch (err) {
  ...
  useEffect(() => { handleDeleteClassRef.current = handleDeleteClass })
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "e") {
        e.preventDefault()
        if (!showEditClass && !showEditDeck && !toDeleteDeck && !showDeleteClass && cls) {
          handleOpenEditClass()
        }
      }
    }
    window.addEventListener("keydown", handler, true)
    return () => window.removeEventListener("keydown", handler, true)
  }, [showEditClass, showEditDeck, toDeleteDeck, showDeleteClass, cls, canEdit])
  ...
  useEffect(() => {
    if (!toDeleteDeck) return
    const handler = (e) => {
      if (e.key === "Escape") { e.stopPropagation(); setToDeleteDeck(null) }
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.key === "Escape") { e.stopPropagation(); setShowDeleteClass(false) }
      if (e.key === "q" || e.key === "Q") { e.stopPropagation(); handleDeleteClassRef.current?.() }
    }
    window.addEventListener("keydown", handler, true)
    return () => window.removeEventListener("keydown", handler, true)
  }, [showDeleteClass])
    ...
    <PageWrapper>
      ...
      <div className={cn("rounded-2xl h-40 mb-6 flex items-center px-8 gap-6 bg-gradient-to-br relative", cls.color)}>
        ...
        <div className="flex gap-2 shrink-0">
          {canEdit && (
            <Button variant="ghost" size="sm" className="bg-white/10 hover:bg-white/20 text-white border-0" onClick={handleOpenEditClass}>
              <Pencil className="h-3.5 w-3.5 mr-1" />Edit
              <kbd className="ml-1.5 text-[10px] bg-white/10 px-1.5 py-0.5 rounded opacity-70">Ctrl+E</kbd>
            </Button>
          )}
          {canDelete && (
            <Button variant="ghost" size="sm" className="bg-white/10 hover:bg-red-500/30 text-white border-0" onClick={() => setShowDeleteClass(true)}>
              <Trash2 className="h-3.5 w-3.5 mr-1" />Delete
            </Button>
          )}
          {canEdit && (
            <Button asChild variant="ghost" className="bg-white/20 hover:bg-white/30 text-white border-0">
              ...
              <CardContent className="p-5 flex flex-col gap-4">
                ...
                <div className="flex gap-2">
                  <Button size="sm" asChild className="flex-1">
                    <Link to={`/decks/${deck._id}`}><Play className="h-3.5 w-3.5 mr-1.5" />View Deck</Link>
                  </Button>
                  {canEdit && (
      ...
      {/* ── Edit Class Dialog ── */}
      <EditClassDialog
        open={showEditClass} onOpenChange={setShowEditClass}
        ...
        editPublic={editPublic} setEditPublic={setEditPublic}
      ...
      <Dialog open={showEditDeck} onOpenChange={setShowEditDeck}>
        <DialogContent className="max-w-lg p-0 overflow-hidden">
          ...
          <div className="px-6 py-5 flex flex-col gap-5 max-h-[65vh] overflow-y-auto" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-foreground">Deck Title</label>
              <Input value={deckTitle} onChange={(e) => setDeckTitle(e.target.value)} placeholder="e.g. Week 1 — Introduction" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-foreground">
                Description <span className="text-muted-foreground font-normal ml-1 text-xs">(optional)</span>
              ...
              <textarea
                ...
                placeholder="What does this deck cover?" rows={3}
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring placeholder:text-muted-foreground"
              />
            </div>
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border">
              ...
              <button
                type="button" onClick={() => setDeckPublic((v) => !v)}
                className={cn("relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0", deckPublic ? "bg-primary" : "bg-muted-foreground/30")}
              >
                <span className={cn("inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-transform", deckPublic ? "translate-x-6" : "translate-x-1")} />
              </button>
            </div>
            {editingDeck && (
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold text-foreground">Cards</label>
                    <Badge variant="secondary" className="text-xs">{editingDeck.cards.length}</Badge>
                  </div>
                  <Link to={`/decks/${editingDeck._id}`} className="flex items-center gap-1 text-xs text-primary hover:underline font-medium" onClick={() => setShowEditDeck(false)}>
                    Edit individual cards →
                  </Link>
                </div>
                <div className="rounded-xl border border-border overflow-hidden">
                  {editingDeck.cards.slice(0, 4).map((card, i) => (
                    <div key={card._id} className={cn("flex items-center gap-3 px-4 py-3 hover:bg-muted/20 transition-colors", i < Math.min(editingDeck.cards.length, 4) - 1 && "border-b border-border")}>
                      ...
                      <div className="flex-1 min-w-0 grid grid-cols-2 gap-3 text-xs">
                        <div className="min-w-0">
                          <p className="text-[9px] font-bold uppercase tracking-wide text-primary mb-0.5">Front</p>
                          <p className="text-foreground font-medium truncate">{card.front}</p>
                        </div>
                        <div className="min-w-0">
                          <p className="text-[9px] font-bold uppercase tracking-wide text-purple-400 mb-0.5">Back</p>
                  ...
                  {editingDeck.cards.length > 4 && (
                    <div className="px-4 py-3 bg-muted/20 text-center">
                      <Link to={`/decks/${editingDeck._id}`} className="text-xs text-primary font-medium hover:underline" onClick={() => setShowEditDeck(false)}>
                        +{editingDeck.cards.length - 4} more cards — click to view all
                      </Link>
                    </div>
                  )}
                </div>
                <p className="text-xs text-muted-foreground">
                  ...
                  <Link to={`/decks/${editingDeck._id}`} className="text-primary hover:underline font-medium" onClick={() => setShowEditDeck(false)}>View Deck</Link>
                </p>
              </div>
            )}
          </div>
          <div className="px-6 py-4 border-t border-border flex justify-between items-center bg-muted/20">
            <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground" asChild onClick={() => setShowEditDeck(false)}>
              <Link to={editingDeck ? `/decks/${editingDeck._id}` : "#"}>
                <BookOpen className="h-3.5 w-3.5 mr-1.5" />Open Full Deck
              </Link>
            </Button>
            <div className="flex gap-2">
        ...
        <DialogContent className="max-w-sm p-0 overflow-hidden">
          <div className="bg-destructive/10 border-b border-destructive/20 px-6 py-5">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-destructive/15 rounded-xl"><AlertTriangle className="h-5 w-5 text-destructive" /></div>
              <div>
                <DialogTitle className="text-base font-bold text-destructive">Delete Subject</DialogTitle>
                <p className="text-xs text-destructive/70 mt-0.5">This action cannot be undone</p>
              </div>
            </div>
          </div>
          <div className="px-6 py-5">
            ...
            <div className="flex items-center gap-3 p-3.5 bg-muted/40 rounded-xl border border-border">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {decks.length} deck{decks.length !== 1 ? "s" : ""} and all cards will be removed
                </p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground mt-3">All decks and flashcards inside this subject will be permanently deleted.</p>
          </div>
          <div className="px-6 py-4 border-t border-border flex justify-end gap-2 bg-muted/20">
            <Button variant="outline" onClick={() => setShowDeleteClass(false)}>
              Keep Subject <kbd className="ml-2 text-[10px] bg-muted px-1.5 py-0.5 rounded opacity-70">Esc</kbd>
            </Button>
            <Button variant="destructive" onClick={() => handleDeleteClassRef.current?.()}>
              <Trash2 className="h-4 w-4 mr-1.5" />Delete Subject
              <kbd className="ml-2 text-[10px] bg-white/20 px-1.5 py-0.5 rounded opacity-70">Q</kbd>
            </Button>
          </div>
        </DialogContent>
