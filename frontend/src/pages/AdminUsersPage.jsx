...
import { Search, ShieldCheck, Ban, UserX } from 'lucide-react'
import { cn }              from '@/lib/utils'

const ROLE_FILTERS   = ['all', 'student', 'creator', 'admin']
...
export default function AdminUsersPage() {
  ...
  const { users, isLoading, promoteUser, toggleSuspend, removeUser } = useAdminUsers({
    role: role === 'all' ? '' : role,
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage roles, suspensions, and bans.</p>
        </div>
              ...
              <tbody className="divide-y">
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Badge variant={user.status === 'suspended' ? 'destructive' : 'outline'} className="text-xs capitalize">
                        {user.status}
                      </Badge>
                    </td>
                      ...
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        {user.role === 'student' && (
                          <Button
                            ...
                            onClick={() => promoteUser(user._id, 'creator')}
                          >
                            <ShieldCheck className="h-3.5 w-3.5 mr-1" />Promote
                          </Button>
                        )}
                        {/* Demote to student */}
                        {user.role === 'creator' && (
                          <Button
                            ...
                            onClick={() => promoteUser(user._id, 'student')}
                          >
                            Demote
                          </Button>
                        )}
      ...
      <Dialog open={!!toBan} onOpenChange={() => setToBan(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-semibold text-foreground">{toBan?.name}</span> (@{toBan?.username})?
