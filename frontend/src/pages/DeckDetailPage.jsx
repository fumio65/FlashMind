...
import { updateDeck, updateCard, deleteCard, deleteDeck, getDeckStats } from "@/features/classes/api/classes";
import {
  ...
  return (
    <div className="mt-1">
      {image ? (
        ...
        <button type="button" onClick={() => fileRef.current?.click()} className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg border border-dashed border-border text-xs text-muted-foreground hover:border-primary hover:text-primary transition-colors">
...
export default function DeckDetailPage() {
  ...
  const isOwner = deck
    ? (
        deck.owner?._id?.toString() === currentUser?._id?.toString() ||
        deck.owner?.toString()      === currentUser?._id?.toString()
      )
    : false
  ...
  // Fetch deck stats (mastery) per user
  useEffect(() => {
    if (!deck?._id) return
    getDeckStats(deck._id)
      .then(setDeckStats)
      .catch(console.error)
  }, [deck?._id])
  ...
  const handleSaveDeck = async () => {
    ...
    try {
      await updateDeck(deck._id, { title: deckTitle, description: deckDesc, isPublic: deckPublic })
    } catch (err) {
  ...
  const handleSaveCard = async () => {
    ...
    setEditingCard(null)
    try {
      await updateCard(editingCard._id, { front: cardFront, back: cardBack, frontImage: cardFrontImage, backImage: cardBackImage })
    } catch (err) {
  ...
  const handleDeleteCard = async () => {
    ...
    try {
      await deleteCard(toDeleteCard._id)
    } catch (err) {
  ...
  return (
    <PageWrapper>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        ...
        {/* Mastery card — per user, per deck */}
        <Card>
