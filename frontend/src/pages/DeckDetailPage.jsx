...
export default function DeckDetailPage() {
  ...
  const handleDeleteDeck = async () => {
    ...
    try {
      ...
      navigate(
        deck.class ? `/classes/${deck.class._id ?? deck.class}` : "/dashboard",
      );
    } catch (err) {
  ...
  const handleDeleteCard = async () => {
    setDeck((prev) => ({
      ...prev,
      cards: prev.cards.filter((c) => c._id !== toDeleteCard._id),
    }));
    ...
    <PageWrapper>
      ...
      <Button variant="ghost" size="sm" asChild className="mb-4 -ml-2">
        <Link
          to={
            deck.class
              ? `/classes/${deck.class._id ?? deck.class}`
              : "/dashboard"
          }
        >
            ...
            {deck.cards.map((card, i) => (
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-3 flex-1 text-sm">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-purple-400 uppercase tracking-wide mb-1">
                      Back
                    </p>
            ...
            <div className="flex items-center gap-3 p-3.5 bg-muted/40 rounded-xl border border-border">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {deck?.cards?.length !== 1 ? "s" : ""} will be removed
                </p>
              </div>
            </div>
      ...
      <Dialog open={!!toDeleteCard} onOpenChange={() => setToDeleteCard(null)}>
        <DialogContent className="max-w-sm p-0 overflow-hidden">
          ...
          <div className="px-6 py-4 border-t border-border flex justify-end gap-2 bg-muted/20">
            ...
            <Button
              variant="destructive"
              onClick={() => handleDeleteCardRef.current?.()}
            >
              <Trash2 className="h-4 w-4 mr-1.5" />
              Delete Card
              <kbd className="ml-2 text-[10px] bg-white/20 px-1.5 py-0.5 rounded opacity-70">
